...
def set_parameters_for_training(
    ...
    from flw_scripts.radar_scene_dataset_loader import RadarSceneGraphDataset
    ...
    print("Config optimizer: ",config_obj.optim)
    print("Config optimizer type: ",type(config_obj.optim))
    ...
    if config_obj.optim == 'adamw' or 'adam': optimizer = torch.optim.AdamW(params, lr=initial_lr, weight_decay=wt_decay)
    else : optimizer = None
    print("Config optimizer: ",config_obj.optim)
 ...
 """
    """ dataset_train = RadarSceneGraphDataset(
        metadatset = train_dataset_metadata, 
        data_rootdir = dataset_rootdir, 
        grid_obj = grid_obj,
        config_obj = config_obj,
        device = device) """
    dataset_train = RadarSceneGraphDataset(train_dataset_dir)
    ...
    """
    """dataset_val = RadarSceneGraphDataset(
        ...
        device = device) 
    """
    dataset_val = RadarSceneGraphDataset(val_dataset_dir)
