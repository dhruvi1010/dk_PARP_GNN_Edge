...
def plot_meas_and_gt_labels_cluster_centers(
    ...
    gt_labels, all_labels, grid_obj,figsize):
    ...
    for i in range(unique_id.shape[0]):
        id = unique_id[i]
        flag = gt_labels == id
        ax.scatter(meas_px[flag], meas_py[flag], 10, color=colors[id.astype(int) % len(colors)], marker='.', label=all_labels[id.astype(int)])
    ...
    # Use grid_obj to define axis limits
    ax.set_xlim(grid_obj.min_x, grid_obj.max_x)
    ax.set_ylim(grid_obj.min_y, grid_obj.max_y)

    plt.title('Radar Point Cloud with Ground Truth Clusters')
    plt.show()
