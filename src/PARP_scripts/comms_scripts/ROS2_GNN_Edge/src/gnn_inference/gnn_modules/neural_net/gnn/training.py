...
def train_model(
    ...
    weight_dir = create_weight_dir(model_weights_dir)
    ...
    best_model_path = os.path.join(weight_dir, "best_model.pt")
    ...
    for iter_train_outer in range(iter_start_offset, max_iters):
        ...
        if graph_features is not None:
            ...
            if isinstance(labels['cluster_node_idx'], list):
                cluster_lengths = [len(x) if isinstance(x, list) else 1 for x in graph_features['node_features_dyn']]
                split_cluster_node_idx = []
                i = 0
                for length in cluster_lengths:
                    split_cluster_node_idx.append(labels['cluster_node_idx'][i:i+length][0])
                    i += length
                labels['cluster_node_idx'] = split_cluster_node_idx
            ...
            with torch.no_grad():
                for iter_val, (graph_features, labels) in enumerate(dataloader_val):
                    if graph_features is not None:
                        ...
                        if iter_val % log_period == 0:
                            loss_str = f"[Iter {iter_val}][loss: {total_loss.item():.5f}]"
                            for key, value in loss.items():
                                loss_str += f"[{key}: {value.item():.5f}]"
                            print(loss_str)

            train_loss_tb, *_ = loss_tracker.compute_avg_training_loss()
            val_loss_tb, *_ = loss_tracker.compute_avg_val_loss()

            print(f"train_losses_tb : {train_loss_tb:.5f}")          
            print(f"val_losses_tb   : {val_loss_tb:.5f}")

            Total_Loss = {'train': train_loss_tb, 'val': val_loss_tb}
            tb_writer.add_scalars('Total_Loss', Total_Loss, iter_train_outer)

            # Write losses for each task
            train_loss_tb, node_cls_train_loss_tb, node_reg_train_loss_tb, edge_cls_train_loss_tb, obj_cls_train_loss_tb = loss_tracker.compute_avg_training_loss()
            val_loss_tb, node_cls_val_loss_tb, node_reg_val_loss_tb, edge_cls_val_loss_tb, obj_cls_val_loss_tb = loss_tracker.compute_avg_val_loss()
            tb_writer.add_scalars('Loss_Node_Segmentation', {'train': node_cls_train_loss_tb, 'val': node_cls_val_loss_tb}, iter_train_outer)
            tb_writer.add_scalars('Loss_Node_Offset', {'train': node_reg_train_loss_tb, 'val': node_reg_val_loss_tb}, iter_train_outer)
            tb_writer.add_scalars('Loss_Edge_Classification', {'train': edge_cls_train_loss_tb, 'val': edge_cls_val_loss_tb}, iter_train_outer)
            tb_writer.add_scalars('Loss_Object_Classification', {'train': obj_cls_train_loss_tb, 'val': obj_cls_val_loss_tb}, iter_train_outer)

            # Write accuracies
            node_cls_train_acc_tb, edge_cls_train_acc_tb, obj_cls_train_acc_tb = acc_tracker.compute_avg_training_acc()
            node_cls_val_acc_tb, edge_cls_val_acc_tb, obj_cls_val_acc_tb = acc_tracker.compute_avg_val_acc()

            tb_writer.add_scalars('Acc_Node_Segmentation', {'train': node_cls_train_acc_tb, 'val': node_cls_val_acc_tb}, iter_train_outer)
            tb_writer.add_scalars('Acc_Edge_Classification', {'train': edge_cls_train_acc_tb, 'val': edge_cls_val_acc_tb}, iter_train_outer)
            tb_writer.add_scalars('Acc_Object_Classification', {'train': obj_cls_train_acc_tb, 'val': obj_cls_val_acc_tb}, iter_train_outer)

            # === Early Stopping and LR Reduction ===
            if val_loss_tb < best_val_loss:
                best_val_loss = val_loss_tb
                epochs_no_improve = 0
                torch.save(detector.state_dict(), best_model_path)
                print(f"✅ New best model saved at iteration {iter_train_outer} with val loss {val_loss_tb:.6f}")
            else:
                epochs_no_improve += 1
                print(f"⚠️ No improvement for {epochs_no_improve} validations.")

                if epochs_no_improve == lr_patience:
                    for param_group in optimizer.param_groups:
                        param_group['lr'] *= lr_reduce_factor
                    print(f"📉 Learning rate reduced to {param_group['lr']:.8f} due to plateau.")

                if epochs_no_improve >= patience:
                    print(f"🛑 Early stopping triggered after {patience} validations with no improvement.")
                    print(f"Loading best model from {best_model_path}...")
                    detector.load_state_dict(torch.load(best_model_path))
                    return  # early exit
...
class LossTracker:
    ...
    def append_training_loss_for_tb(self, total_loss, losses):
        self.train_losses_tb.append(total_loss.detach().item()) 
